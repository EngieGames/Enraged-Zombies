...
public class BEnragedSpeedEntity extends Monster {
	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(EnragedZombiesModEntities.B_ENRAGED_SPEED.get(), 20, 1, 4));
	}

	public BEnragedSpeedEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnragedZombiesModEntities.B_ENRAGED_SPEED.get(), world);
	}

	public BEnragedSpeedEntity(EntityType<BEnragedSpeedEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 10;
		setNoAi(false);
	...
	public static void init() {
		SpawnPlacements.register(EnragedZombiesModEntities.B_ENRAGED_SPEED.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(EnragedZombiesModEntities.B_ENRAGED_SPEED.get(), 180);
	}
