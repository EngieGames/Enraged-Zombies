...
public class EnragedBoomEntity extends Monster {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(EnragedZombiesModEntities.ENRAGED_BOOM.get(), 20, 1, 4));
	}

	public EnragedBoomEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnragedZombiesModEntities.ENRAGED_BOOM.get(), world);
	}
	...
	public static void init() {
		SpawnPlacements.register(EnragedZombiesModEntities.ENRAGED_BOOM.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(EnragedZombiesModEntities.ENRAGED_BOOM.get(), 180);
	}
