...
public class OEnragedSpeedEntity extends Monster {
	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(EnragedZombiesModEntities.O_ENRAGED_SPEED.get(), 20, 1, 4));
	}

	public OEnragedSpeedEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnragedZombiesModEntities.O_ENRAGED_SPEED.get(), world);
	}

	public OEnragedSpeedEntity(EntityType<OEnragedSpeedEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 10;
		setNoAi(false);
	...
	public static void init() {
		SpawnPlacements.register(EnragedZombiesModEntities.O_ENRAGED_SPEED.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(EnragedZombiesModEntities.O_ENRAGED_SPEED.get(), 180);
	}
